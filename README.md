...
bashpython collect/collecting.py --out raw_custom/subject01 --duration 30 --fps 30
...
Record with collect/collecting.py (produces a UBFC-format subject folder).
Point root in main.py's CONFIG at that folder, and give it a distinct source_tag
(e.g. own) so it appears separately in the report. The folder name must match.
Run preprocess, then run.
...
those terms — review them and state your license below. 
