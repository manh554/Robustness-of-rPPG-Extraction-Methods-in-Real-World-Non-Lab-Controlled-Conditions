...
the lab. The full write-up is in the final report; this is the short, plot-driven version.
...
![Cross-dataset robustness: MAE and Pearson by method on the self-collected set](figures/results_summary.png)

*Left: heart-rate MAE (lower is better). Right: HR Pearson correlation (higher is better).
Blue = training-free, red = supervised. Evaluated on the same 11 self-collected recordings.*
...
(e.g. 81 -> 39 bpm, 116 -> 43 bpm).

![Recovered pulse waveforms, supervised model vs contact reference](figures/waveforms_supervised.png)

*Prediction vs contact reference for three self-collected recordings. On the favorable case the
rate is recovered (60 vs 61 bpm); on the others the dominant frequency is biased low.*

The power spectra confirm this is a **peak-selection** issue: on favorable recordings the predicted
spectrum has a single peak at the true rate, while on failing recordings it becomes broadband and
the peak-picker locks onto a spurious low-frequency component.

![Power spectra of predicted signals, favorable vs failing recordings](figures/spectra_supervised.png)

*Green dashed = reference HR; red dotted = selected spectral peak (prediction).*

## Agreement analysis

Bland-Altman plots show the same story. Training-free CHROM (in-domain, UBFC-rPPG) has a small
bias (~4 bpm) with points near zero, while supervised DeepPhys on the self-collected data shows a
large bias (~24 bpm) and much wider limits of agreement, with predictions collapsing toward a
narrow low range.

![Bland-Altman agreement analysis](figures/bland_altman.png)

*Note: the CHROM panel is an in-domain reference (held-out UBFC-rPPG) and is not directly
comparable to the supervised self-collected panels.*
...
directory; the summary figure above is generated from the numbers in the table. Drop the chosen
PNGs into `figures/` with the names referenced here, or update the paths to match your run output.
